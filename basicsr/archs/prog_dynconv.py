...
class IDynamicDWConv(nn.Module):
    def __init__(self,
        ...
        Block1 = [blocks.ResBlock(channels, channels, kernel_size=kernel_size, stride=1, groups=conv_group)
                  for _ in range(n_blocks)]
        Block2 = [blocks.ResBlock(channels, channels, kernel_size=kernel_size, stride=1, groups=conv_group)
                  for _ in range(n_blocks)]
        Block3 = [blocks.ResBlock(channels, channels, kernel_size=kernel_size, stride=1, groups=conv_group)
                  for _ in range(n_blocks)]
